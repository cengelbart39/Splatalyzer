...
final class SubStatsTests: XCTestCase {
    ...
    func test_StatHelper_subVelocity_notNil() throws {
        ...
        var velocity = StatHelper.subVelocity(ap: self.ap, subInfo: data)
        XCTAssertNotNil(velocity)
                
        // Autobomb
        data = try TestHelper.getSubData(for: .autobomb)
        velocity = StatHelper.subVelocity(ap: self.ap, subInfo: data)
        XCTAssertNotNil(velocity)
    ...
    func test_StatHelper_subPhaseDuration_true_nil() throws {
        ...
        // Angle Shooter
        var data = try TestHelper.getSubData(for: .angleShooter)
        var duration = StatHelper.subPhaseDuration(ap: self.ap, subInfo: data, first: true)
        XCTAssertNil(duration)
                
        // Autobomb
        data = try TestHelper.getSubData(for: .autobomb)
        duration = StatHelper.subPhaseDuration(ap: self.ap, subInfo: data, first: true)
        XCTAssertNil(duration)
    ...
    func test_StatHelper_subPhaseDuration_false_nil() throws {
        ...
        // Angle Shooter
        var data = try TestHelper.getSubData(for: .angleShooter)
        var duration = StatHelper.subPhaseDuration(ap: self.ap, subInfo: data, first: false)
        XCTAssertNil(duration)
                
        // Autobomb
        data = try TestHelper.getSubData(for: .autobomb)
        duration = StatHelper.subPhaseDuration(ap: self.ap, subInfo: data, first: false)
        XCTAssertNil(duration)
    ...
    func test_StatHelper_subMarkingSeconds_notNil() throws {
        ...
        // Angle Shooter
        var data = try TestHelper.getSubData(for: .angleShooter)
        var seconds = StatHelper.subMarkingSeconds(ap: self.ap, subInfo: data)
        XCTAssertNotNil(seconds)
        
        // Ink Mine
        data = try TestHelper.getSubData(for: .inkMine)
        seconds = StatHelper.subMarkingSeconds(ap: self.ap, subInfo: data)
        XCTAssertNotNil(seconds)
    ...
    func test_StatHelper_subMarkingSeconds_nil() throws {
        ...
        var duration = StatHelper.subMarkingSeconds(ap: self.ap, subInfo: data)
        XCTAssertNil(duration)
        
        // Burst Bomb
        data = try TestHelper.getSubData(for: .burstBomb)
        duration = StatHelper.subMarkingSeconds(ap: self.ap, subInfo: data)
        XCTAssertNil(duration)
    ...
    func test_StatHelper_subMarkingRadius_nil() throws {
        ...
        // Angle Shooter
        var data = try TestHelper.getSubData(for: .angleShooter)
        var radius = StatHelper.subMarkingRadius(ap: self.ap, subInfo: data)
        XCTAssertNil(radius)
        
        // Autobomb
        data = try TestHelper.getSubData(for: .autobomb)
        radius = StatHelper.subMarkingRadius(ap: self.ap, subInfo: data)
        XCTAssertNil(radius)
    ...
    func test_StatHelper_subExplosionRadius_nil() throws {
        ...
        // Angle Shooter
        var data = try TestHelper.getSubData(for: .angleShooter)
        var radius = StatHelper.subExplosionRadius(ap: self.ap, subInfo: data)
        XCTAssertNil(radius)
        
        // Autobomb
        data = try TestHelper.getSubData(for: .autobomb)
        radius = StatHelper.subExplosionRadius(ap: self.ap, subInfo: data)
        XCTAssertNil(radius)
    ...
    func test_StatHelper_subHp_nil() throws {
        ...
        // Angle Shooter
        var data = try TestHelper.getSubData(for: .angleShooter)
        var hp = StatHelper.subHp(ap: self.ap, subInfo: data)
        XCTAssertNil(hp)
        
        // Autobomb
        data = try TestHelper.getSubData(for: .autobomb)
        hp = StatHelper.subHp(ap: self.ap, subInfo: data)
        XCTAssertNil(hp)
    ...
    func test_StatHelper_quickSuperJumpBoost_notNil() throws {
        ...
        // Angle Shooter
        var data = try TestHelper.getSubData(for: .angleShooter)
        var velocity = StatHelper.quickSuperJumpBoost(ap: self.ap, subInfo: data)
        XCTAssertNotNil(velocity)
                
        // Autobomb
        data = try TestHelper.getSubData(for: .autobomb)
        velocity = StatHelper.quickSuperJumpBoost(ap: self.ap, subInfo: data)
        XCTAssertNotNil(velocity)
